      ...
      <div className="mt-6 space-y-2">
        {loading ? (
          ...
          <div className="card text-center py-10">
            ...
            <p className="text-sm text-muted mt-1">Crea tu primer curso y publícalo cuando esté listo.</p>
          </div>
        ) : (
          courses.map((c) => (
            <div key={c.id} className="card flex items-center justify-between gap-3">
              <div>
                <p className="font-medium">
                  {c.title}{' '}
                  <span className="text-xs text-muted">(${Number(c.price_usdc).toFixed(2)} USDC)</span>
                </p>
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button onClick={() => setToDelete(c)} disabled={busy} className="btn-ghost text-status-danger">
                  Eliminar
                </button>
              </div>
            </div>
