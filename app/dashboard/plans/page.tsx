  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="mt-6 space-y-2">
        {loading ? (
          <p className="text-sm text-ink/60">Cargando planes…</p>
        ) : plans.length === 0 ? (
            ...
            <div key={p.id} className="card flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs mt-1" role="status" aria-live="polite">
                  {!p.active ? (
                    <span className="text-ink/40">desactivado</span>
                  ) : p.onchain_synced ? (
                    <span
                      className="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-0.5 text-emerald-600"
                    ...
                    <span
                      className="inline-flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-0.5 text-amber-600 animate-pulse"
              ...
              {p.active && (
                <div className="flex gap-2 shrink-0">
                  ...
                  <button onClick={() => setToDeactivate(p)} disabled={busy} className="btn-ghost text-red-600">
                    Desactivar
                  </button>
                </div>
              )}
