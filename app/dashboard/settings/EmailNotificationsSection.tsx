...
export default function EmailNotificationsSection() {
  ...
  return (
    <div className="card mt-6">
      ...
      {loading ? (
        <p className="text-sm text-ink/60 mt-3">Cargando…</p>
      ) : (
        <>
          ...
          {verified ? (
            <p className="text-xs text-green-600 mt-2">✓ Email verificado</p>
          ) : (
          ...
          {status && (
            <p className={`text-sm mt-3 ${statusOk ? 'text-green-600' : 'text-amber-600'}`} role="status">
              {status}
            </p>
          )}
