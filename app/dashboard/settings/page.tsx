...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="card mt-6 border-status-danger/30">
        <h2 className="font-semibold text-status-danger">Eliminar mi cuenta</h2>
        ...
        <button onClick={() => setConfirmDelete(true)} disabled={busy}
          className="btn bg-danger hover:opacity-90 text-ink mt-4">
          {busy ? 'Procesando…' : 'Eliminar mi cuenta'}
        </button>
        {error && <p className="text-sm text-status-danger mt-3" role="alert">{error}</p>}
      </div>
